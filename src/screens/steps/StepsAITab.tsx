...
import { buildWalkingInsights } from '../../data/stepsInsights';
...
export default function StepsAITab() {
  const { sum, goal, streak, entries, loading } = useStepsToday();
  const [refreshTick, setRefreshTick] = useState(0);

  const insights = useMemo(
    () => buildWalkingInsights({ sum, goal, streak, entries }),
    [sum, goal, streak, entries, refreshTick]
  );
