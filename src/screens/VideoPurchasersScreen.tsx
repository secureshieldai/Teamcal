...
type Props = NativeStackScreenProps<RootStackParamList, 'VideoPurchasers'>;

export default function VideoPurchasersScreen({ route, navigation }: Props) {
  ...
  const purchasers = (video.metadata as VideoMetadata)?.purchasers || [];
  const filtered = search.trim() ? purchasers.filter(p => p.name.toLowerCase().includes(search.toLowerCase())) : purchasers;
  const totalRevenue = purchasers.reduce((sum, p) => sum + p.amount, 0);
  ...
  return (
    <SafeAreaView style={s.safe} edges={['top']}>
      <View style={s.header}>
        ...
        <Text style={s.headerTitle}>View Purchasers</Text>
        <TouchableOpacity hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
          <Ionicons name="filter-outline" size={20} color={colors.textPrimary} />
        </TouchableOpacity>
      </View>
      <View style={s.statsRow}>
        <View style={s.statBox}><Text style={s.statValue}>{purchasers.length}</Text><Text style={s.statLabel}>Total Purchasers</Text></View>
        <View style={s.statBox}><Text style={s.statValue}>${totalRevenue.toFixed(2)}</Text><Text style={s.statLabel}>Total Revenue</Text></View>
      </View>
      <View style={s.searchRow}>
        <Ionicons name="search-outline" size={16} color={colors.textMuted} />
        <TextInput style={s.searchInput} value={search} onChangeText={setSearch} placeholder="Search purchasers…" placeholderTextColor={colors.textMuted} />
      </View>
      <FlatList
        data={filtered}
        keyExtractor={p => p.id}
        ...
        ListEmptyComponent={
          <View style={s.emptyState}>
            ...
            <Text style={s.emptyTitle}>No purchasers yet</Text>
            <Text style={s.emptyText}>This list populates automatically once viewers purchase this video.</Text>
          </View>
        }
        renderItem={({ item }) => (
          <View style={s.row}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={s.meta}>{new Date(item.date).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</Text>
            </View>
