...
export type PdfEdition={id:string;name:string;fileUrl:string;fileName:string;fileSize:number;releaseNotes:string;notifyBuyers:boolean;grantToExistingBuyers:boolean;createdAt:string;status:'draft'|'published'};
export type PdfMetadata={
  ...
  // Buyer preview (14E)
  previewType?:'first-pages'|'specific-pages'|'custom-content'|'no-preview';previewPages?:number;specificPages?:string;
  customPreviewFile?:{fileUrl:string;fileName:string;fileSize:number;fileType:string;pages?:number};
  // Pricing & discounts (14F)
  isFree?:boolean;discountType?:'percent'|'fixed';discountValue?:number;discountCode?:string;discountActive?:boolean;discountUsageLimit?:number;discountUses?:number;discountAppliesTo?:string;
  // Editions (14C)
  editions?:PdfEdition[];
  // Showcase (14K)
  addToShowcase?:boolean;downloadsEnabled?:boolean;
};
...
export type MembershipContentItem={id:string;title:string;type:'post'|'pdf'|'video'|'image'|'link'|'audio'|'poll'|'course';description?:string;url?:string;access:string;status:'published'|'draft'|'scheduled';scheduledFor?:string;createdAt:string};
export type ModeratorPermissions={content:boolean;members:boolean;events:boolean};
export type AutoDmMessage={enabled:boolean;text:string;attachments:{name:string;url:string;type:string}[];updatedAt?:string};
export type OnboardingVideo={enabled:boolean;url?:string;fileName?:string;title?:string;description?:string;status:'draft'|'published'};
export type PixelPlatform='meta'|'tiktok'|'snapchat'|'google';
export type PixelConfig={id:string;status:'connected'|'invalid'|'not-connected';events:string[];updatedAt?:string};
export type MembershipMetadata={groupId?:string;profileImage?:string;banner?:string;category?:string;subcategory?:string;rules?:string;valueProposition?:string;audience?:string;memberReceives?:string;welcomeMessage?:string;faqs?:{question:string;answer:string}[];language?:string;privacy?:string;discoverability?:string;memberApproval?:string;postingPermission?:string;commentPermission?:string;dmPermission?:string;moderatorPermission?:string;eventPermission?:string;contentRules?:string;pricingModel?:'free'|'lifetime'|'recurring'|'tiers';currency?:string;lifetimePrice?:number;launchPrice?:number;couponCodes?:string[];lifetimeTerms?:string;monthlyPrice?:number;quarterlyPrice?:number;sixMonthPrice?:number;annualPrice?:number;trial?:string;trialPlans?:string[];paymentRequiredForTrial?:boolean;trialReminder?:boolean;autoRenew?:boolean;repeatTrials?:boolean;tiers?:MembershipTier[];benefits?:string[];members?:MembershipMember[];events?:MembershipEvent[];resources?:{id:string;title:string;type:string;access:string}[];scheduledPosts?:{id:string;title:string;date:string;access:string}[];testimonials?:string[];linkBehavior?:string;
  ...
  // Moderators (16B.6)
  moderatorPermissions?:Record<string,ModeratorPermissions>;
  // Plugins (16C)
  pluginLeaderboard?:boolean;pluginStreak?:boolean;streakAction?:string;instantApproval?:boolean;
  autoDmMessage?:AutoDmMessage;onboardingVideo?:OnboardingVideo;
  // Tracking pixels (16D)
  pixels?:Partial<Record<PixelPlatform,PixelConfig>>;
};
...
export type StoreOrder={id:string;created_at:string;total_amount:number;currency:string;status:string;items:{id:string;title:string;price:number;quantity:number}[];buyer?:{id:string;name?:string;email?:string;avatar?:string}};
export type StoreCustomer={id:string;name?:string;email?:string;avatar?:string;orders:number;spent:number;lastOrderAt:string};
...
export const earnService = {
  ...
  async createStoreProduct(storeId:string,value:Record<string,unknown>){const {data}=await apiClient.post<{success:boolean;product:Record<string,unknown>;asset:EarnAsset}>(`/earn/stores/${storeId}/products`,value);return data;},
  async getStoreOrders(storeId:string){const {data}=await apiClient.get<{success:boolean;orders:StoreOrder[]}>(`/earn/stores/${storeId}/orders`);return data.orders;},
  async getStoreCustomers(storeId:string){const {data}=await apiClient.get<{success:boolean;customers:StoreCustomer[]}>(`/earn/stores/${storeId}/customers`);return data.customers;},
