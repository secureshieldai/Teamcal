...
async function articleEngagement(req,res,next){try{const articleId=req.params.id;const {data:entries,error}=await supabase.from('tracker_entries').select('*').in('tracker',['article-like','article-comment']).contains('meta',{articleId}).order('ts',{ascending:true});if(error)throw error;const comments=(entries||[]).filter(x=>x.tracker==='article-comment');const ids=[...new Set(comments.map(x=>x.user_id))];const {data:users}=ids.length?await supabase.from('users').select('id,name,avatar').in('id',ids):{data:[]};const byId=Object.fromEntries((users||[]).map(x=>[x.id,x]));const likes=(entries||[]).filter(x=>x.tracker==='article-like');res.json({success:true,likes:likes.length,liked:likes.some(x=>x.user_id===req.user.id),comments:comments.map(x=>({id:x.id,name:byId[x.user_id]?.name||'Member',avatar:byId[x.user_id]?.avatar||'',time:new Date(x.ts).toLocaleString(),text:x.meta.text,likes:0,userId:x.user_id}))});}catch(e){next(e);}}
...
async function addArticleComment(req,res,next){try{const text=String(req.body.text||'').trim();if(!text)return res.status(400).json({success:false,message:'Comment is required'});const {data,error}=await supabase.from('tracker_entries').insert({user_id:req.user.id,tracker:'article-comment',ts:Date.now(),value:0,meta:{articleId:req.params.id,text}}).select().single();if(error)throw error;res.status(201).json({success:true,comment:{id:data.id,name:req.user.name||'You',avatar:req.user.avatar||'',time:'now',text,likes:0,userId:req.user.id}});}catch(e){next(e);}}
...
module.exports = { getFeed, searchUsers, getProfile, getLeaderboard, toggleFriend, getFriends, getFriendsProgress, toggleFollow, getCreators, getSocialBlogs, getSocialBlog, getSocialVideos, getStories, articleEngagement, toggleArticleLike, addArticleComment, deleteArticleComment, toggleBlogFollow };
