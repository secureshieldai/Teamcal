...
async function getSocialBlogs(req,res,next){try{const limit=Math.min(Number(req.query.limit)||15,30);const {data,error}=await supabase.from('articles').select('id,blog_id,title,cover,excerpt,category,read_minutes,views,earned,created_at,user:user_id(id,name,avatar,verified)').eq('status','published').order('created_at',{ascending:false}).limit(limit);if(error)throw error;res.json({success:true,blogs:data||[]});}catch(e){next(e);}}
